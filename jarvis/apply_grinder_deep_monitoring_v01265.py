...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"ZBRANO v0.12.65 expected one {label} marker; found {count}")
...
'''
    ...
    backend = backend.replace('version="0.12.64"', 'version="0.12.65"')
    backend = backend.replace('"version": "0.12.64"', '"version": "0.12.65"')
    backend = backend.replace('"X-ZBRANO-Frontend-Version": "0.12.64"', '"X-ZBRANO-Frontend-Version": "0.12.65"')
    backend = backend.replace('"name": "ZBRANO Developer Mode", "version": "0.12.64"', '"name": "ZBRANO Developer Mode", "version": "0.12.65"')
    frontend = frontend.replace("HUD 0.12.64", "HUD 0.12.65")

    required = (
        "GRINDER_MONITOR_TOOLS", "grinder_monitor_worker", "/api/grinder-monitor/status",
        "zbrano-grinder-deep-monitor", 'version="0.12.65"',
    )
    missing = [marker for marker in required if marker not in backend]
    if missing or "HUD 0.12.65" not in frontend:
        raise RuntimeError("ZBRANO v0.12.65 verification failed: " + ", ".join(missing))
