(() => {
  ...
  const wakeBrowserActivate=document.getElementById("wake-browser-activate"),wakeLocalActivate=document.getElementById("wake-local-activate"),wakeConversationEnabled=document.getElementById("wake-conversation-enabled"),wakeConversationStop=document.getElementById("wake-conversation-stop");
  ...
  wakeShadowEnabled.checked=localStorage.getItem("zbrano_wake_shadow_enabled")==="true";wakeShadowThreshold.value=localStorage.getItem("zbrano_wake_shadow_threshold")||"0.50";wakeShadowThresholdValue.textContent=Number(wakeShadowThreshold.value).toFixed(2);wakeLocalActivate.checked=localStorage.getItem("zbrano_wake_local_activate")==="true";wakeBrowserActivate.checked=!wakeLocalActivate.checked;wakeConversationEnabled.checked=localStorage.getItem("zbrano_wake_conversation")==="true";wakeConversationStop.value=localStorage.getItem("zbrano_wake_conversation_stop")||"ok thank you";renderWakeShadow();
  ...
  function selectWakeDetectionMethod(useLocal){wakeLocalActivate.checked=useLocal;wakeBrowserActivate.checked=!useLocal;if(useLocal&&!wakeShadowEnabled.checked){wakeShadowEnabled.checked=true;localStorage.setItem("zbrano_wake_shadow_enabled","true")}localStorage.setItem("zbrano_wake_local_activate",String(useLocal));stopWake();if(wakeEnabled.checked)scheduleWake();else renderWakeShadow()}
  wakeLocalActivate.addEventListener("change",()=>{if(wakeLocalActivate.checked)selectWakeDetectionMethod(true)});
  wakeBrowserActivate.addEventListener("change",()=>{if(wakeBrowserActivate.checked)selectWakeDetectionMethod(false)});
  ...
  wakeShadowEnabled.addEventListener("change",()=>{if(!wakeShadowEnabled.checked){wakeLocalActivate.checked=false;wakeBrowserActivate.checked=true;localStorage.setItem("zbrano_wake_local_activate","false")}localStorage.setItem("zbrano_wake_shadow_enabled",String(wakeShadowEnabled.checked));stopWake();if(wakeEnabled.checked)scheduleWake();else renderWakeShadow()});
