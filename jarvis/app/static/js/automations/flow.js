...
(() => {
  ...
  function node(kind,index,label,title,detail) {
    ...
    element.append(kicker,heading);if(String(detail??"").trim()){const description=document.createElement("small");description.textContent=detail;element.append(description)}return element;
  }
  ...
  function conditionCard(item,index,entityName){
    const kind=item.kind||"entity";
    if(kind==="time_window")return node("context",index,`IF ${index+1}`,`${text(item.start_time,"start")} – ${text(item.end_time,"end")}`,"");
  ...
  function branchStage(branches,entityName,interactive=false){
    ...
    const heading=document.createElement("div");heading.className="automation-flow-stage-heading";heading.textContent="DO THE FIRST MATCHING PATH";
