(() => {
  ...
  function actionStepValid(item){const kind=item.kind||"service";return kind==="delay"?Number(item.delay_seconds)>0:kind==="wait_state"?Boolean(item.entity_id):Boolean(item.entity_id&&item.service)}
  function actionStepHtml(item,attributes){
    const kind=item.kind||"service",attr=field=>`${attributes} data-action-field="${field}"`;
    const type=`<select ${attr("kind")}><option value="service"${kind==="service"?" selected":""}>Service action</option><option value="delay"${kind==="delay"?" selected":""}>Delay</option><option value="wait_state"${kind==="wait_state"?" selected":""}>Wait until</option></select>`;
  ...
  function renderWorkflowInspector(root){
    ...
    section.addEventListener("input",event=>{const field=event.target.dataset.actionField||event.target.dataset.triggerField||event.target.dataset.conditionField,index=Number(event.target.dataset.workflowIndex);if(!field||!items[index])return;items[index][field]=field==="weekdays"?parseWeekdays(event.target.value):field.endsWith("seconds")||field.endsWith("minutes")?Number(event.target.value||0):event.target.value;if(field==="kind")renderStudioInspector();renderEditorFlow()});
    ...
    section.addEventListener("click",event=>{const add=event.target.closest("[data-workflow-add]"),remove=event.target.closest("[data-workflow-remove]");if(add){items.push(collection==="actions"?{kind:"service",entity_id:"",service:"",service_data:{},delay_seconds:0,wait_operator:"equals",wait_value:"",timeout_seconds:30}:collection==="triggers"?{kind:"entity",entity_id:"",operator:"changes_to",value:"",for_seconds:0,weekdays:[],at:"",sun_event:"sunrise",offset_minutes:0,interval_minutes:5,one_time_at:""}:{kind:"entity",entity_id:"",operator:"equals",value:"",weekdays:[],start_time:"",end_time:"",sun_state:"below_horizon"});renderStudioInspector();renderEditorFlow()}else if(remove){items.splice(Number(remove.dataset.workflowRemove),1);renderStudioInspector();renderEditorFlow()}});
  }
  ...
  function renderSuggestions(){
    ...
    const visible=(state.suggestions||[]).filter(item=>!["dismissed"].includes(item.status)&&item.delivery_notification_center!==false).slice(0,30);
  ...
  function renderLibrary(){
    ...
    for(const item of state.automations){
      ...
      const feedback=item.feedback_memory||{},feedbackTotal=Number(feedback.dismissals||0)+Number(feedback.approvals||0)+Number(feedback.manual_resolutions||0),resetLearning=feedbackTotal?`<button type="button" data-auto-reset-learning="${esc(item.id)}">Reset learning</button>`:"";
      ...
      row.innerHTML=`<div class="autonomy-draft-head"><div><strong>${esc(item.name)}</strong><div>${esc(item.objective)}</div></div><div class="autonomy-draft-actions">${primaryAction}${resetLearning}<button type="button" data-auto-delete="${esc(item.id)}">Delete</button></div></div><div class="autonomy-tags">${tags.map(tag=>`<span>${esc(tag)}</span>`).join("")}</div>`;
  ...
  function automationRequestBody(){
    ...
    body.reoffer_delta=Number($("automation-reoffer-delta").value||0);body.reset_delta=Number($("automation-reset-delta").value||0);const workflow=editorSnapshot();if(workflow.actions.length&&workflow.actions[0].entity_id===body.action_entity)workflow.actions[0].service_data=actionData;Object.assign(body,{triggers:workflow.triggers,conditions:workflow.conditions,condition_mode:workflow.condition_mode,actions:workflow.actions,branches:workflow.branches});return body;
  }
