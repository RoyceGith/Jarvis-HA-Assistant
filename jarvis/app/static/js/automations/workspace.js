(() => {
  ...
  function addBranchCondition(branchIndex,insertionIndex){
    const conditions=workflowDraft.branches[branchIndex]?.conditions;if(!conditions)return false;if(conditions.length>=20){$("automation-studio-state").textContent="This branch already has its maximum of 20 conditions.";return false}const target=Math.max(0,Math.min(insertionIndex,conditions.length));conditions.splice(target,0,{kind:"entity",entity_id:"",operator:"equals",value:"",for_seconds:0,weekdays:[],start_time:"",end_time:"",sun_state:"below_horizon"});selectedStudioNode="decision";selectedFlowCard={kind:"branch-condition",index:target,branchIndex};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("branch-condition",target,branchIndex));$("automation-studio-state").textContent="Condition block added to the selected branch. Complete its settings before saving.";return true;
  }
