(() => {
  ...
  function renderLibrary(){
    ...
    for(const item of state.automations){
      ...
      const reasoning=["deferred","paused_failure"].includes(item.status)?item.last_deferred_reason:item.status==="satisfied"?item.last_satisfied_reason:"";if(reasoning)row.insertAdjacentHTML("beforeend",`<small><strong>Why ${esc(item.status)}:</strong> ${esc(reasoning)}</small>`);
