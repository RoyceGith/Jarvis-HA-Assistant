(() => {
  ...
  const studioPanels={
    ...
    context:{title:"Condition",help:"Choose what must be true before ZBRANO continues.",fields:[["automation-presence","Presence condition"],["automation-signals","Additional condition entities"],["automation-context-notes","Condition notes"]]},
  ...
  function conditionStepHtml(item,attributes){
    ...
    const type=`<select ${attr("kind")}><option value="entity"${kind==="entity"?" selected":""}>Entity state</option><option value="entity_compare"${kind==="entity_compare"?" selected":""}>Compare entities</option><option value="time_window"${kind==="time_window"?" selected":""}>Time window</option><option value="weekday"${kind==="weekday"?" selected":""}>Weekdays</option><option value="sun"${kind==="sun"?" selected":""}>Sun state</option></select>`;
    if(kind==="entity")return `${type}<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="sensor.entity"><select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select><input ${attr("value")} value="${esc(item.value||"")}" placeholder="value"><input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}" aria-label="Condition duration seconds">`;
    if(kind==="entity_compare")return `${type}<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="Entity value to compare"><select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select><input ${attr("compare_entity_id")} list="automation-entity-options" value="${esc(item.compare_entity_id||"")}" placeholder="Comparison entity"><input ${attr("compare_attribute")} value="${esc(item.compare_attribute||"")}" placeholder="Attribute, e.g. temperature"><input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}" aria-label="Comparison duration seconds">`;
  ...
  function newBranchCondition(template="entity"){return {kind:["entity","entity_compare","time_window","weekday","sun"].includes(template)?template:"entity",entity_id:"",operator:template==="entity_compare"?"above":"equals",value:"",compare_entity_id:"",compare_attribute:"",for_seconds:0,weekdays:[],start_time:"",end_time:"",sun_state:"below_horizon"}}
