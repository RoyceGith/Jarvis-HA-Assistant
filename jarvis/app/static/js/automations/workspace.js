(() => {
  ...
  function renderStudioInspector(){
    ...
    const policy=$("automation-execution-policy").value,controlDevice=$("automation-risk").value!=="informational",mayAct=["inherit","approval_required","autonomous"].includes(policy);
    ...
    for(const [id,labelText] of inspectorFields(panelConfig)){
      if(selectedStudioNode==="details"&&((actionOnlyFields.has(id)&&(!controlDevice||!mayAct))||(id==="automation-reversible-only"&&(!controlDevice||policy!=="autonomous"))))continue;
      ...
      const synchronize=()=>{if(source.type==="checkbox")source.checked=control.checked;else source.value=control.value;source.dispatchEvent(new Event("input",{bubbles:true}));if(id==="automation-trigger-kind"||id==="automation-trigger-operator"||id==="automation-execution-policy"||id==="automation-risk"||id==="automation-require-presence")renderStudioInspector()};
