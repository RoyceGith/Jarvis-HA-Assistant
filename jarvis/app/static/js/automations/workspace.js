(() => {
  ...
  const studioPanels={
    ...
    trigger:{title:"When",help:"Choose the events that wake this automation so ZBRANO can evaluate its IF / ELSE IF paths.",fields:[["automation-trigger-kind","Event type"],["automation-trigger-entity","Entity to monitor"],["automation-trigger-operator","Event rule"],["automation-trigger-value","Value"],["automation-trigger-for","Sustain for seconds"],["automation-trigger-at","Local time"],["automation-trigger-weekdays","Selected weekdays"],["automation-trigger-sun-event","Sun event"],["automation-trigger-sun-offset","Sun offset minutes"],["automation-trigger-interval","Repeat every minutes"],["automation-trigger-one-time","One-time local date and time"]]},
    context:{title:"Shared condition",help:"Add a full condition that must be true for every path. Add path-specific conditions inside each IF or ELSE IF path.",fields:[["automation-context-notes","Optional notes"]]},
  ...
  function entityLabel(id){const entity=entityMap.get(id),name=String(entity?.friendly_name||"").trim();return name&&name!==id?`${name} · ${id}`:id}
