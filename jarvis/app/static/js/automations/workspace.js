(() => {
  ...
  function activate(){
    for(const id of ["chat-panel","entities-panel","settings-panel","plugins-panel","files-panel","contacts-panel","calendar-panel","about-panel","developer-panel","automations-panel"]){document.getElementById(id)?.classList.toggle("hidden",id!=="automations-panel")}
    for(const id of ["chat-tab","entities-tab","settings-tab","plugins-tab","files-tab","calendar-tab","about-tab","developer-tab","automations-tab"]){document.getElementById(id)?.classList.toggle("active",id==="automations-tab")}
  }
  ...
  function recoverLocalEditorDraft(payload){
    ...
    const saved=new Date(Number(payload.saved_at||0));$("automation-studio-state").textContent=`Recovered unsaved flow from ${saved.toLocaleString(window.ZbranoI18n?.locale || undefined)}. New flow discards it.`;
  }
  ...
  function renderActivity(){
    ...
    for(const item of [...automations].sort((left,right)=>Number(automationNeedsAttention(right))-Number(automationNeedsAttention(left))||Number(Boolean(right.enabled))-Number(Boolean(left.enabled))||String(left.name||"").localeCompare(String(right.name||"")))){
      ...
      const row=document.createElement("div");row.className="automation-health-row";row.dataset.tone=attention?"attention":item.enabled?"healthy":"off";row.innerHTML=`<div><strong>${esc(item.name||"Unnamed automation")}</strong><span>${esc(status)}</span></div><button type="button" data-activity-open-automation="${esc(item.id)}">Open</button><small>${last?`${esc(friendlyDecision(last.outcome))} · ${new Date(Number(last.created_at||0)*1000).toLocaleString(window.ZbranoI18n?.locale || undefined)}`:item.enabled?"Waiting for its first matching event":"Enable it when you are ready"}</small>`;health.appendChild(row);
    }
  }
  ...
  function renderTimeline(){
    ...
    for(const item of state.timeline){const row=document.createElement("div");row.className="autonomy-event";row.innerHTML=`<strong>${esc(item.title)}</strong>${item.detail?`<span>${esc(item.detail)}</span>`:""}<time>${new Date(Number(item.created_at||0)*1000).toLocaleString(window.ZbranoI18n?.locale || undefined)}</time>`;root.appendChild(row)}
  }
  ...
  document.addEventListener("click",event=>{const other=event.target.closest?.("#chat-tab,#entities-tab,#settings-tab,#plugins-tab,#files-tab,#contacts-tab,#calendar-tab,#about-tab,#developer-tab");if(other){panel.classList.add("hidden");tab.classList.remove("active")}},true);
