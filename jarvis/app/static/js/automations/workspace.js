(() => {
  ...
  function startElseIfFlow(){
    ...
    workflowDraft.branches=[
      {name:"IF",suggestion:firstMessage,message_enabled:Boolean(firstMessage),condition_mode:"all",...branchDeliveryDefaults(),conditions:firstChecks.length?firstChecks:[newBranchCondition()],actions:firstActions},
      {name:"ELSE IF",suggestion:"",message_enabled:false,condition_mode:"all",...branchDeliveryDefaults(),conditions:[newBranchCondition()],actions:[]},
    ];
  ...
  function addStudioBlock(kind,insertionIndex=null){
    ...
    if(kind==="trigger"){
      ...
      const target=insertionIndex==null?workflowDraft.branches.length:Math.max(0,Math.min(insertionIndex,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:"ELSE IF",suggestion:"",message_enabled:false,condition_mode:"all",...branchDeliveryDefaults(),conditions:[condition()],actions:[]});selectedFlowCard={kind,index:target};
    }else if(kind==="action"){
  ...
  function renderEditorValidation(){
    ...
    const kinds=new Set(issues.map(issue=>issue.kind));for(const node of $("automation-flow-preview").querySelectorAll("[data-flow-kind]")){const kind=["branch-action","branch-condition","branch-message"].includes(node.dataset.flowKind)?"decision":node.dataset.flowKind,invalid=kinds.has(kind);node.classList.toggle("has-validation-error",invalid);node.setAttribute("aria-invalid",String(invalid))}
