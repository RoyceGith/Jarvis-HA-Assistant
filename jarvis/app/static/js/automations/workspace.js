(() => {
  ...
  function renderBranchInspector(root){
    ...
    const cards=workflowDraft.branches.map((branch,branchIndex)=>{
      ...
      const pathWord=branchIndex===0?"IF":"OTHERWISE IF";
      return `<article class="automation-branch-card"><div class="automation-branch-path-label">${pathWord}</div><div class="automation-workflow-head"><input data-branch-name="${branchIndex}" value="${esc(branch.name||`Outcome ${branchIndex+1}`)}" aria-label="Outcome name"><button type="button" data-branch-remove="${branchIndex}">Remove</button></div><label>What should ZBRANO say here?<textarea data-branch-suggestion="${branchIndex}" maxlength="1000" placeholder="Message to show when this outcome matches">${esc(branch.suggestion||"")}</textarea></label><label>Use this outcome when<select data-branch-mode="${branchIndex}"><option value="all"${branch.condition_mode!=="any"?" selected":""}>ALL checks below pass (AND)</option><option value="any"${branch.condition_mode==="any"?" selected":""}>ANY check below passes (OR)</option></select></label><small>No checks means OTHERWISE: use this when nothing above matches.</small>${conditions}<button type="button" data-branch-add-item="conditions" data-branch-index="${branchIndex}">+ Add a check</button><strong>THEN DO</strong>${actions}<button type="button" data-branch-add-item="actions" data-branch-index="${branchIndex}">+ Add a task</button></article>`;
    }).join("");
    section.innerHTML=`<div class="automation-workflow-head"><strong>Different outcomes</strong><button type="button" data-branch-add>+ Add outcome</button></div><small>ZBRANO reads these from left to right and uses the first one that matches. Your normal Then path stays active when no outcomes are added.</small>${cards||'<small class="automation-workflow-empty">No extra outcomes yet. Your simple When → Then flow remains active.</small>'}`;root.append(section);
    ...
    section.addEventListener("click",event=>{const add=event.target.closest("[data-branch-add]"),remove=event.target.closest("[data-branch-remove]"),addItem=event.target.closest("[data-branch-add-item]"),removeItem=event.target.closest("[data-branch-remove-item]");if(add){workflowDraft.branches.push({name:`Outcome ${workflowDraft.branches.length+1}`,suggestion:"",condition_mode:"all",conditions:[],actions:[]})}else if(remove){workflowDraft.branches.splice(Number(remove.dataset.branchRemove),1)}else if(addItem){const branch=workflowDraft.branches[Number(addItem.dataset.branchIndex)],collection=addItem.dataset.branchAddItem;branch[collection].push(collection==="actions"?newActionTask():newBranchCondition())}else if(removeItem){workflowDraft.branches[Number(removeItem.dataset.branchIndex)][removeItem.dataset.branchRemoveItem].splice(Number(removeItem.dataset.itemIndex),1)}else return;renderStudioInspector();renderEditorFlow();commitEditorHistory()});
  }
  ...
  function addBranchCondition(branchIndex,insertionIndex,template="entity"){
    const conditions=workflowDraft.branches[branchIndex]?.conditions;if(!conditions)return false;if(conditions.length>=20){$("automation-studio-state").textContent="This branch already has its maximum of 20 conditions.";return false}const target=Math.max(0,Math.min(insertionIndex,conditions.length));conditions.splice(target,0,newBranchCondition(template));selectedStudioNode="decision";selectedFlowCard={kind:"branch-condition",index:target,branchIndex};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("branch-condition",target,branchIndex));$("automation-studio-state").textContent=`${({entity:"Entity state",entity_compare:"Compare entities",time_window:"Time window",weekday:"Weekdays",sun:"Sun state"})[template]||"IF"} condition added to the selected branch. Complete its settings before saving.`;return true;
  }
  ...
  function addBranchPath(){
    if(workflowDraft.branches.length>=10){$("automation-studio-state").textContent="A flow supports up to 10 outcomes.";return false}const fallbackIndex=workflowDraft.branches.findIndex((branch,index)=>index===workflowDraft.branches.length-1&&!(branch.conditions||[]).length),selectedAfter=selectedFlowCard.kind==="decision"?selectedFlowCard.index+1:workflowDraft.branches.length,target=Math.max(0,Math.min(fallbackIndex>=0?Math.min(selectedAfter,fallbackIndex):selectedAfter,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:`Outcome ${target+1}`,suggestion:"",condition_mode:"all",conditions:[newBranchCondition()],actions:[]});selectedStudioNode="decision";selectedFlowCard={kind:"decision",index:target,branchIndex:null};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("decision",target));$("automation-studio-state").textContent="Outcome added before OTHERWISE. Choose when it should run and what it should do.";return true;
  }
  ...
  function deleteBranchPath(index){
    const branches=workflowDraft.branches;if(!branches[index])return false;if(branches.length===1){$("automation-studio-state").textContent="Keep at least one outcome, or turn off outcomes in Advanced settings.";return false}const removed=branches.splice(index,1)[0],target=Math.max(0,Math.min(index,branches.length-1));selectedStudioNode="decision";selectedFlowCard={kind:"decision",index:target,branchIndex:null};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("decision",target));$("automation-studio-state").textContent=`${removed.name||"Outcome"} removed. Use Undo to restore it.`;return true;
  }
  function addStudioBlock(kind,insertionIndex=null){
    ...
    if(kind==="trigger"){
      ...
      if(workflowDraft.branches.length>=10){$("automation-studio-state").textContent="A flow supports up to 10 outcomes.";return}
      const target=insertionIndex==null?workflowDraft.branches.length:Math.max(0,Math.min(insertionIndex,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:`Outcome ${workflowDraft.branches.length+1}`,suggestion:"",condition_mode:"all",conditions:[condition()],actions:[action()]});selectedFlowCard={kind,index:target};
    }else if(kind==="action"){
  ...
  function editorValidationIssues(){
    ...
    const validateCondition=(condition,issueKind="context",prefix="Condition")=>{const kind=condition.kind||"entity";if(["entity","entity_compare"].includes(kind)&&!condition.entity_id)add(issueKind,`${prefix}: choose an entity`);else if(kind==="entity_compare"&&!condition.compare_entity_id)add(issueKind,`${prefix}: choose a comparison entity`);else if(kind==="time_window"&&(!condition.start_time||!condition.end_time))add(issueKind,`${prefix}: complete the time window`);else if(kind==="weekday"&&!(condition.weekdays||[]).length)add(issueKind,`${prefix}: select at least one weekday`)};
