(() => {
  ...
  function automationRequestBody(){
    ...
    body.suggestion_timeout_minutes=Number($("automation-suggestion-timeout").value||30);body.reoffer_delta=Number($("automation-reoffer-delta").value||0);body.reset_delta=Number($("automation-reset-delta").value||0);const workflow=editorSnapshot();if(workflow.actions.length&&workflow.actions[0].entity_id===body.action_entity)workflow.actions[0].service_data=actionData;Object.assign(body,{triggers:workflow.triggers,conditions:workflow.conditions,condition_mode:workflow.condition_mode,actions:workflow.actions,branches:workflow.branches});return body;
  }
