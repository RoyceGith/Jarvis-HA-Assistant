(() => {
  ...
  function renderBranchInspector(root){
    ...
    const cards=workflowDraft.branches.map((branch,branchIndex)=>{
      ...
      const fallback=branchIndex===workflowDraft.branches.length-1&&!(branch.conditions||[]).length,pathWord=fallback?"IF NO OTHER RESULT MATCHES":`RESULT ${branchIndex+1}`;
      return `<article class="automation-branch-card"><div class="automation-branch-path-label">${pathWord}</div><div class="automation-workflow-head"><label class="automation-outcome-name"><span>Name this result</span><input data-branch-name="${branchIndex}" value="${esc(branch.name||`Result ${branchIndex+1}`)}" aria-label="Result name"></label><button type="button" data-branch-remove="${branchIndex}">Remove</button></div><label>Message for this result (optional)<textarea data-branch-suggestion="${branchIndex}" maxlength="1000" placeholder="Leave blank to use the main message">${esc(branch.suggestion||"")}</textarea></label><label>Use this result when<select data-branch-mode="${branchIndex}"><option value="all"${branch.condition_mode!=="any"?" selected":""}>Every check below is true</option><option value="any"${branch.condition_mode==="any"?" selected":""}>At least one check below is true</option></select></label><small>${fallback?"This is the fallback result. It is used only when none of the results above match.":"Describe the situation below. ZBRANO will test these checks together."}</small><strong>WHEN</strong>${conditions}<button type="button" data-branch-add-item="conditions" data-branch-index="${branchIndex}">+ Add another check</button><strong>THEN</strong>${actions}<button type="button" data-branch-add-item="actions" data-branch-index="${branchIndex}">+ Add a task for this result</button></article>`;
    }).join("");
    section.innerHTML=cards?`<div class="automation-workflow-head"><strong>Your different results</strong><button type="button" data-branch-add>+ Another result</button></div><small>ZBRANO checks these in order and uses the first result whose situation is true.</small>${cards}`:`<div class="automation-outcome-choice"><span class="automation-outcome-choice-icon" aria-hidden="true">&#8646;</span><strong>Should this automation do something different in another situation?</strong><small>Most automations do not need this. Your simple When → Then flow already works.</small><button type="button" data-branch-add>Add a different result</button></div>`;root.append(section);
  ...
  function addBranchPath(){
    if(workflowDraft.branches.length>=10){$("automation-studio-state").textContent="A flow supports up to 10 different results.";return false}const fallbackIndex=workflowDraft.branches.findIndex((branch,index)=>index===workflowDraft.branches.length-1&&!(branch.conditions||[]).length),selectedAfter=selectedFlowCard.kind==="decision"?selectedFlowCard.index+1:workflowDraft.branches.length,target=Math.max(0,Math.min(fallbackIndex>=0?Math.min(selectedAfter,fallbackIndex):selectedAfter,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:`Result ${target+1}`,suggestion:"",condition_mode:"all",conditions:[newBranchCondition()],actions:[]});selectedStudioNode="decision";selectedFlowCard={kind:"decision",index:target,branchIndex:null};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("decision",target));$("automation-studio-state").textContent="Different result added. Describe when it applies and what should happen.";return true;
  }
