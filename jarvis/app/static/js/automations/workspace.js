(() => {
  ...
  function inspectorFields(panelConfig){
    ...
    if(selectedStudioNode==="decision"){
      ...
      return panelConfig.fields.filter(([id])=>!(id==="automation-proposal"&&(silent||hasBranches))&&!((silent||hasBranches)&&["automation-delivery-voice","automation-delivery-center","automation-delivery-push"].includes(id)));
    }
  ...
  function renderBranchInspector(root){
    ...
    const cards=workflowDraft.branches.map((branch,branchIndex)=>[branch,branchIndex]).filter(([,branchIndex])=>branchIndex===Math.max(0,Math.min(selectedBranchIndex,workflowDraft.branches.length-1))).map(([branch,branchIndex])=>{
      ...
      const message=showMessages?`<label>Message for this path<textarea data-branch-suggestion="${branchIndex}" maxlength="1000" placeholder="Write the message for this situation">${esc(branch.suggestion||"")}</textarea></label><div class="automation-branch-delivery"><strong>Send this message</strong><label class="is-check"><input type="checkbox" data-branch-delivery="delivery_voice" data-branch-index="${branchIndex}"${branchDeliveryValue(branch,"delivery_voice")?" checked":""}> Say this message aloud</label><label class="is-check"><input type="checkbox" data-branch-delivery="delivery_notification_center" data-branch-index="${branchIndex}"${branchDeliveryValue(branch,"delivery_notification_center")?" checked":""}> Show in ZBRANO notifications</label><label class="is-check"><input type="checkbox" data-branch-delivery="delivery_ha_push" data-branch-index="${branchIndex}"${branchDeliveryValue(branch,"delivery_ha_push")?" checked":""}> Send to Home Assistant notifications</label></div>`:"<small>This path monitors quietly and sends no message.</small>";
      return `<article class="automation-branch-card"><div class="automation-branch-path-label">${pathWord}</div><div class="automation-workflow-head"><strong>${pathWord} message</strong><button type="button" data-branch-remove="${branchIndex}">Remove path</button></div>${message}${fallback?"<small>This fallback is used only when no path above matches.</small>":""}</article>`;
    }).join("");
  ...
  function startElseIfFlow(){
    ...
    workflowDraft.branches=[
      {name:"IF",suggestion:firstMessage,condition_mode:"all",...branchDeliveryDefaults(),conditions:firstChecks.length?firstChecks:[newBranchCondition()],actions:firstActions},
      {name:"ELSE IF",suggestion:"",condition_mode:"all",...branchDeliveryDefaults(),conditions:[newBranchCondition()],actions:[]},
    ];
  ...
  function addStudioBlock(kind,insertionIndex=null){
    ...
    if(kind==="trigger"){
      ...
      const target=insertionIndex==null?workflowDraft.branches.length:Math.max(0,Math.min(insertionIndex,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:"ELSE IF",suggestion:"",condition_mode:"all",...branchDeliveryDefaults(),conditions:[condition()],actions:[]});selectedFlowCard={kind,index:target};
    }else if(kind==="action"){
