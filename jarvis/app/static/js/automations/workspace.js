(() => {
  ...
  function renderLibrary(){
    ...
    for(const item of visible){
      ...
      const actionSummary=item.action_service&&item.action_entity?`${entityLabel(item.action_entity)} — ${item.action_service.replaceAll("_"," ")}`:"Only show the message";
  ...
  function updateStudioGuide(issues){
    ...
    const contextCount=($("automation-presence").value.trim()?1:0)+$("automation-signals").value.split(/[,\n]/).map(value=>value.trim()).filter(Boolean).length+workflowDraft.conditions.length;
