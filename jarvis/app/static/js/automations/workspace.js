(() => {
  ...
  const studioPanels={
    details:{title:"1. Setup & safety",help:"Name this automation, choose who must be present, then set exactly how much authority this rule has.",fields:[["automation-name","Automation name"],["automation-objective","What should it help with?"],["automation-require-presence","Require presence"],["automation-presence","Who must be present?"],["automation-execution-policy","What may ZBRANO do?"],["automation-risk","What does this automation affect?"],["automation-max-actions","Most times this may run in one hour"],["automation-reversible-only","Only allow automatic actions that can be undone"],["automation-notify-action","Tell me after it runs"],["automation-enabled","Enable automation on saving"]]},
  ...
  const studioStepNames={details:"Setup & safety",trigger:"When",context:"And",action:"Then",decision:"Else if"};
  ...
  function renderStudioInspector(){
    ...
    for(const [id,labelText] of inspectorFields(panelConfig)){
      ...
      const synchronize=()=>{if(source.type==="checkbox")source.checked=control.checked;else source.value=control.value;source.dispatchEvent(new Event("input",{bubbles:true}));if(id==="automation-trigger-kind"||id==="automation-trigger-operator"||id==="automation-execution-policy"||id==="automation-require-presence")renderStudioInspector()};
  ...
  function conditionStepHtml(item,attributes){
    ...
    const labelled=(label,control)=>`<label><span>${label}</span>${control}</label>`;
    const type=labelled("Check",`<select ${attr("kind")}><option value="entity"${kind==="entity"?" selected":""}>A device or sensor value</option><option value="entity_compare"${kind==="entity_compare"?" selected":""}>Compare two device values</option><option value="time_window"${kind==="time_window"?" selected":""}>The time is within a range</option><option value="weekday"${kind==="weekday"?" selected":""}>Today is one of these days</option><option value="sun"${kind==="sun"?" selected":""}>The sun is up or down</option></select>`);
    if(kind==="entity")return `${type}${labelled("Device or sensor",`<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="Choose a device or sensor">`)}${labelled("Which value?",`<input ${attr("attribute")} list="automation-attribute-options" value="${esc(item.attribute||"")}" placeholder="State, temperature, humidity...">`)}${labelled("Must be",`<select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select>`)}${labelled("Compared with",`<input ${attr("value")} value="${esc(item.value||"")}" placeholder="For example: home, off, cooling, 26">`)}${labelled("Keep true for (seconds)",`<input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}">`)}`;
    if(kind==="entity_compare")return `${type}${labelled("First device or sensor",`<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="Choose the value to test">`)}${labelled("Its value",`<input ${attr("attribute")} list="automation-attribute-options" value="${esc(item.attribute||"")}" placeholder="State, temperature, humidity...">`)}${labelled("Must be",`<select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select>`)}${labelled("Other device or sensor",`<input ${attr("compare_entity_id")} list="automation-entity-options" value="${esc(item.compare_entity_id||"")}" placeholder="Choose the comparison device">`)}${labelled("Its value",`<input ${attr("compare_attribute")} list="automation-attribute-options" value="${esc(item.compare_attribute||"")}" placeholder="State, temperature, humidity...">`)}${labelled("Keep true for (seconds)",`<input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}">`)}`;
  ...
  function renderBranchInspector(root){
    ...
    const selectedBranchIndex=["branch-action","branch-condition"].includes(selectedFlowCard.kind)?Number(selectedFlowCard.branchIndex||0):selectedFlowCard.kind==="decision"?Number(selectedFlowCard.index||0):0;
    const cards=workflowDraft.branches.map((branch,branchIndex)=>[branch,branchIndex]).filter(([,branchIndex])=>branchIndex===Math.max(0,Math.min(selectedBranchIndex,workflowDraft.branches.length-1))).map(([branch,branchIndex])=>{
      ...
      if(selectedFlowCard.kind==="branch-action"&&selectedFlowCard.branchIndex===branchIndex&&branch.actions?.[selectedFlowCard.index])return `<article class="automation-branch-card"><div class="automation-branch-path-label">${branchIndex?"ELSE IF":"IF"} · THEN TASK</div><div class="automation-workflow-step">${actionStepHtml(branch.actions[selectedFlowCard.index],`data-branch-collection="actions" data-branch-index="${branchIndex}" data-item-index="${selectedFlowCard.index}"`)}<button type="button" data-branch-remove-item="actions" data-branch-index="${branchIndex}" data-item-index="${selectedFlowCard.index}">Remove task</button></div></article>`;
      ...
      return `<article class="automation-branch-card"><div class="automation-branch-path-label">${pathWord}</div><div class="automation-workflow-head"><label class="automation-outcome-name"><span>Path label</span><input data-branch-name="${branchIndex}" value="${esc(branch.name||pathWord)}" aria-label="Path label"></label><button type="button" data-branch-remove="${branchIndex}">Remove</button></div><label>What ZBRANO should say on this path (optional)<textarea data-branch-suggestion="${branchIndex}" maxlength="1000" placeholder="Leave blank to use the main message">${esc(branch.suggestion||"")}</textarea></label><label>This path runs when<select data-branch-mode="${branchIndex}"><option value="all"${branch.condition_mode!=="any"?" selected":""}>Every check below is true</option><option value="any"${branch.condition_mode==="any"?" selected":""}>At least one check below is true</option></select></label><small>${fallback?"This is the fallback result. It is used only when none of the results above match.":"Describe the situation below. ZBRANO will test these checks together."}</small><strong>WHEN</strong>${conditions}<button type="button" data-branch-add-item="conditions" data-branch-index="${branchIndex}">+ Add AND check</button><strong>THEN</strong>${actions}<button type="button" data-branch-add-item="actions" data-branch-index="${branchIndex}">+ Add THEN task</button></article>`;
    }).join("");
    section.innerHTML=cards?`<div class="automation-workflow-head"><strong>IF / ELSE IF paths</strong><button type="button" data-branch-add>+ ELSE IF</button></div><small>The first matching path runs. Each path keeps its own checks, message, and tasks.</small>${cards}`:`<div class="automation-outcome-choice"><span class="automation-outcome-choice-icon" aria-hidden="true">&#8646;</span><strong>Add an ELSE IF path</strong><small>Your current And checks and Then tasks become the first IF path. The new path stays separate.</small><button type="button" data-branch-add>Add ELSE IF</button></div>`;root.append(section);
    section.addEventListener("input",event=>{const branchIndex=Number(event.target.dataset.branchIndex),branch=workflowDraft.branches[branchIndex];if(event.target.hasAttribute("data-branch-name")){workflowDraft.branches[Number(event.target.dataset.branchName)].name=event.target.value;renderEditorFlow();scheduleEditorHistory();return}if(event.target.hasAttribute("data-branch-suggestion")){workflowDraft.branches[Number(event.target.dataset.branchSuggestion)].suggestion=event.target.value;renderEditorFlow();scheduleEditorHistory();return}const collection=event.target.dataset.branchCollection,itemIndex=Number(event.target.dataset.itemIndex),field=event.target.dataset.actionField||event.target.dataset.conditionField,dataField=event.target.dataset.actionDataField;if(!branch||!collection||(!field&&!dataField))return;const item=branch[collection][itemIndex];if(dataField)item.service_data={...(item.service_data||{}),[dataField]:Number(event.target.value)};else item[field]=field==="weekdays"?parseWeekdays(event.target.value):field.endsWith("seconds")||field.endsWith("minutes")?Number(event.target.value||0):event.target.value;if(field==="kind")renderStudioInspector();renderEditorFlow();scheduleEditorHistory()});
    ...
    section.addEventListener("click",event=>{const add=event.target.closest("[data-branch-add]"),remove=event.target.closest("[data-branch-remove]"),addItem=event.target.closest("[data-branch-add-item]"),removeItem=event.target.closest("[data-branch-remove-item]");if(add){addBranchPath();return}else if(remove){workflowDraft.branches.splice(Number(remove.dataset.branchRemove),1)}else if(addItem){const branch=workflowDraft.branches[Number(addItem.dataset.branchIndex)],collection=addItem.dataset.branchAddItem;branch[collection].push(collection==="actions"?newActionTask():newBranchCondition())}else if(removeItem){workflowDraft.branches[Number(removeItem.dataset.branchIndex)][removeItem.dataset.branchRemoveItem].splice(Number(removeItem.dataset.itemIndex),1)}else return;renderStudioInspector();renderEditorFlow();commitEditorHistory()});
  }
  ...
  function startElseIfFlow(){
    const firstChecks=workflowDraft.conditions.splice(0),firstActions=visualFlowItems("action");
    writeFlowSequence("action",[]);
    workflowDraft.branches=[
      {name:"IF",suggestion:"",condition_mode:workflowDraft.condition_mode,conditions:firstChecks.length?firstChecks:[newBranchCondition()],actions:firstActions},
      {name:"ELSE IF",suggestion:"",condition_mode:"all",conditions:[newBranchCondition()],actions:[]},
    ];
    selectedStudioNode="decision";selectedFlowCard={kind:"decision",index:1,branchIndex:null};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("decision",1));$("automation-studio-state").textContent="ELSE IF path added. The existing And checks and Then tasks are connected to the first IF path.";return true;
  }
  function addBranchPath(){
    if(!workflowDraft.branches.length)return startElseIfFlow();
    if(workflowDraft.branches.length>=10){$("automation-studio-state").textContent="A flow supports up to 10 paths.";return false}const fallbackIndex=workflowDraft.branches.findIndex((branch,index)=>index===workflowDraft.branches.length-1&&!(branch.conditions||[]).length),selectedAfter=selectedFlowCard.kind==="decision"?selectedFlowCard.index+1:workflowDraft.branches.length,target=Math.max(0,Math.min(fallbackIndex>=0?Math.min(selectedAfter,fallbackIndex):selectedAfter,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:"ELSE IF",suggestion:"",condition_mode:"all",conditions:[newBranchCondition()],actions:[]});selectedStudioNode="decision";selectedFlowCard={kind:"decision",index:target,branchIndex:null};renderStudioInspector();renderEditorFlow();commitEditorHistory();requestAnimationFrame(()=>focusSelectedFlowEditor("decision",target));$("automation-studio-state").textContent="ELSE IF path added. Add its check and Then task.";return true;
  }
  ...
  function addStudioBlock(kind,insertionIndex=null){
    ...
    if(kind==="trigger"){
      ...
      const target=insertionIndex==null?workflowDraft.branches.length:Math.max(0,Math.min(insertionIndex,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:"ELSE IF",suggestion:"",condition_mode:"all",conditions:[condition()],actions:[]});selectedFlowCard={kind,index:target};
    }else if(kind==="action"){
  ...
  function editorSnapshot(){
    ...
    return {
      ...
      branches:workflowDraft.branches.map(branch=>({...branch,name:(branch.name||"Outcome").trim(),conditions:(branch.conditions||[]).filter(item=>(item.kind||"entity")!=="entity"||item.entity_id),actions:(branch.actions||[]).filter(actionStepValid)})),
    };
  }
  ...
  function editorValidationIssues(){
    ...
    const validateAction=(item,issueKind="action",prefix="Task")=>{const kind=item.kind||"service";if(kind==="service"&&(!item.entity_id||!item.service))add(issueKind,`${prefix}: choose a device and what it should do`);else if(kind==="notification"&&(!item.entity_id||!item.notification_message))add(issueKind,`${prefix}: choose where to send it and enter a message`);else if(kind==="delay"&&Number(item.delay_seconds)<1)add(issueKind,`${prefix}: wait for at least one second`);else if(kind==="wait_state"&&!item.entity_id)add(issueKind,`${prefix}: choose a device or sensor`)};
    for(const action of workflowDraft.actions)validateAction(action);
    for(const branch of workflowDraft.branches){for(const condition of branch.conditions||[])validateCondition(condition,"decision",`${branch.name||"Outcome"} check`);for(const action of branch.actions||[])validateAction(action,"decision",`${branch.name||"Outcome"} task`)}
    return issues;
  }
