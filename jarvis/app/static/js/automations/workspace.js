(() => {
  ...
  function inspectorFields(panelConfig){
    ...
    if(selectedStudioNode==="decision"){
      const silent=["observe","autonomous"].includes($("automation-execution-policy").value),hasBranches=workflowDraft.branches.length>0;
      return panelConfig.fields.filter(([id])=>!(id==="automation-proposal"&&(silent||hasBranches))&&!(silent&&["automation-delivery-voice","automation-delivery-center","automation-delivery-push"].includes(id)));
    }
  ...
  function renderBranchInspector(root){
    ...
    const showMessages=!["observe","autonomous"].includes($("automation-execution-policy").value);
  ...
  function focusSelectedFlowEditor(kind,index,branchIndex=null){
    ...
    else if(kind==="decision")target=root.querySelector(`[data-branch-suggestion="${index}"]`)||root.querySelector(`[data-branch-index="${index}"]`);
  ...
  function startElseIfFlow(){
    ...
    workflowDraft.branches=[
      {name:"IF",suggestion:firstMessage,condition_mode:workflowDraft.condition_mode,conditions:firstChecks.length?firstChecks:[newBranchCondition()],actions:firstActions},
  ...
  function editorValidationIssues(){
    ...
    const branchMessagesRequired=!["observe","autonomous"].includes($("automation-execution-policy").value);
    for(const [branchIndex,branch] of workflowDraft.branches.entries()){const pathName=branchIndex?"ELSE IF":"IF";if(branchMessagesRequired&&!String(branch.suggestion||"").trim())add("decision",`${pathName} path: write its message`);for(const condition of branch.conditions||[])validateCondition(condition,"decision",`${pathName} check`);for(const action of branch.actions||[])validateAction(action,"decision",`${pathName} task`)}
    return issues;
  }
