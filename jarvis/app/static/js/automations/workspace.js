(() => {
  ...
  function conditionStepHtml(item,attributes){
    ...
    const type=labelled("Condition type",`<select ${attr("kind")}><option value="entity"${kind==="entity"?" selected":""}>Entity value</option><option value="entity_compare"${kind==="entity_compare"?" selected":""}>Compare two entity values</option><option value="time_window"${kind==="time_window"?" selected":""}>Time window</option><option value="weekday"${kind==="weekday"?" selected":""}>Weekdays</option><option value="sun"${kind==="sun"?" selected":""}>Sun state</option></select>`);
    if(kind==="entity")return `${type}${labelled("Entity",`<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="Choose an entity">`)}${labelled("Read",`<input ${attr("attribute")} list="automation-attribute-options" value="${esc(item.attribute||"")}" placeholder="State (or attribute name)">`)}${labelled("Comparison",`<select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select>`)}${labelled("Required value",`<input ${attr("value")} value="${esc(item.value||"")}" placeholder="For example: home, off, cooling, 26">`)}${labelled("Must remain true for seconds",`<input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}">`)}`;
    if(kind==="entity_compare")return `${type}${labelled("Left entity",`<input ${attr("entity_id")} list="automation-entity-options" value="${esc(item.entity_id||"")}" placeholder="Choose the value to test">`)}${labelled("Left value",`<input ${attr("attribute")} list="automation-attribute-options" value="${esc(item.attribute||"")}" placeholder="State (or attribute name)">`)}${labelled("Comparison",`<select ${attr("operator")}>${workflowOperatorOptions(item.operator)}</select>`)}${labelled("Right entity",`<input ${attr("compare_entity_id")} list="automation-entity-options" value="${esc(item.compare_entity_id||"")}" placeholder="Choose the comparison entity">`)}${labelled("Right value",`<input ${attr("compare_attribute")} list="automation-attribute-options" value="${esc(item.compare_attribute||"")}" placeholder="State (or attribute, e.g. temperature)">`)}${labelled("Must remain true for seconds",`<input ${attr("for_seconds")} type="number" min="0" max="86400" value="${Number(item.for_seconds||0)}">`)}`;
    if(kind==="time_window")return `${type}<input ${attr("start_time")} type="time" value="${esc(item.start_time||"")}"><input ${attr("end_time")} type="time" value="${esc(item.end_time||"")}">`;
    if(kind==="weekday")return `${type}<input ${attr("weekdays")} value="${esc(formatWeekdays(item.weekdays))}" placeholder="Mon, Tue, Wed">`;
    return `${type}<select ${attr("sun_state")}><option value="below_horizon"${item.sun_state!=="above_horizon"?" selected":""}>After sunset</option><option value="above_horizon"${item.sun_state==="above_horizon"?" selected":""}>After sunrise</option></select>`;
  }
  ...
  function renderBranchInspector(root){
    ...
    section.addEventListener("click",event=>{const add=event.target.closest("[data-branch-add]"),remove=event.target.closest("[data-branch-remove]"),addItem=event.target.closest("[data-branch-add-item]"),removeItem=event.target.closest("[data-branch-remove-item]");if(add){workflowDraft.branches.push({name:`Branch ${workflowDraft.branches.length+1}`,suggestion:"",condition_mode:"all",conditions:[],actions:[]})}else if(remove){workflowDraft.branches.splice(Number(remove.dataset.branchRemove),1)}else if(addItem){const branch=workflowDraft.branches[Number(addItem.dataset.branchIndex)],collection=addItem.dataset.branchAddItem;branch[collection].push(collection==="actions"?newActionTask():newBranchCondition())}else if(removeItem){workflowDraft.branches[Number(removeItem.dataset.branchIndex)][removeItem.dataset.branchRemoveItem].splice(Number(removeItem.dataset.itemIndex),1)}else return;renderStudioInspector();renderEditorFlow();commitEditorHistory()});
  }
  ...
  function addStudioBlock(kind,insertionIndex=null){
    ...
    if(kind==="trigger"){
      ...
      const target=insertionIndex==null?workflowDraft.branches.length:Math.max(0,Math.min(insertionIndex,workflowDraft.branches.length));workflowDraft.branches.splice(target,0,{name:`Branch ${workflowDraft.branches.length+1}`,suggestion:"",condition_mode:"all",conditions:[condition()],actions:[action()]});selectedFlowCard={kind,index:target};
    }else if(kind==="action"){
  ...
  function editorValidationIssues(){
    ...
    for(const branch of workflowDraft.branches){for(const condition of branch.conditions||[])validateCondition(condition,"decision",`${branch.name||"Branch"} condition`);for(const action of branch.actions||[])validateAction(action,"decision",`${branch.name||"Branch"} action`)}
    return issues;
  }
