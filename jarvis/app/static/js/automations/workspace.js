(() => {
  ...
  function renderWorkflowInspector(root){
    ...
    section.addEventListener("input",event=>{const field=event.target.dataset.actionField||event.target.dataset.workflowField,index=Number(event.target.dataset.workflowIndex);if(!field||!items[index])return;items[index][field]=field.endsWith("seconds")?Number(event.target.value||0):event.target.value;if(field==="kind")renderStudioInspector();renderEditorFlow()});
  ...
  function editorSnapshot(){
    ...
    return {
      ...
      branches:workflowDraft.branches.map(branch=>({...branch,name:(branch.name||"Branch").trim(),conditions:(branch.conditions||[]).filter(item=>item.entity_id),actions:(branch.actions||[]).filter(actionStepValid)})),
    };
  }
