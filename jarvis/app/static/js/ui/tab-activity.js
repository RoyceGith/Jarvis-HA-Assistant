(() => {
  ...
  document.addEventListener("click", event => {
    const button = event.target.closest?.(
      "#chat-tab,#files-tab,#plugins-tab,#entities-tab,#contacts-tab,#calendar-tab,#automations-tab,#memory-tab,#settings-tab,#about-tab,[data-auto-view],[data-notification-view],.settings-category-tab[data-settings-target],#plugins-installed-tab,#plugins-browse-tab"
    );
    if (!button) return;
    clear(button);
