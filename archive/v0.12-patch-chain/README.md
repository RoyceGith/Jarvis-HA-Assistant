...
- `zbrano/` contains the 147 Python scripts that previously transformed the original backend and frontend during every image build.
- `tests/` contains the 121 release-specific tests that validated those scripts and their order.

Starting with v0.13.0, the generated v0.12.112 result is checked in under `zbrano/app/` as canonical source. Nothing in this archive is copied into the Home Assistant image or executed by the active build. New development and tests must target canonical source directly.
