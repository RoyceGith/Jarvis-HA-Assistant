...
PATCH = (ROOT / "zbrano/apply_new_chat_draft_fix_v01121.py").read_text(encoding="utf-8")
...
def test_v01121_new_chat_is_unsaved_draft_until_first_message():
    ...
    assert 'zbranoChatSessionId = createSessionId();' in PATCH
    assert 'localStorage.setItem("zbrano_chat_session_id", zbranoChatSessionId);' in PATCH
