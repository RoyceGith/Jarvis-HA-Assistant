...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.1 patch missing: {label}")
...
def patch_index() -> None:
    ...
    marker = 'window.zbranoActivatePanel = activatePanel;'
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError("ZBRANO v0.11.1 verification failed: " + ", ".join(missing))
