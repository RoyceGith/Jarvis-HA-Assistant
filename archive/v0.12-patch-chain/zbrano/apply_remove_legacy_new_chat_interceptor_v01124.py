...
def must(text, marker, label):
    if marker not in text:
        raise RuntimeError(f'ZBRANO v0.11.24 patch missing: {label}')
...
def verify():
    ...
    if 'event.target.closest?.("#new-chat-button, #clear-chat")' in index:
        raise RuntimeError('ZBRANO v0.11.24 verification failed: legacy capture interceptor remains')
    if 'id="clear-chat"' in index:
        raise RuntimeError('ZBRANO v0.11.24 verification failed: legacy clear-chat button remains')
