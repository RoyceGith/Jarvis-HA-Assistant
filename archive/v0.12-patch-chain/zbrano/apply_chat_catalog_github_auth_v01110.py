...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.10 patch missing: {label}")
...
def patch_index() -> None:
    ...
    if runtime_start >= 0:
        ...
        if runtime_end < 0:
            raise RuntimeError("ZBRANO v0.11.10 patch missing: v0.11.9 runtime end")
        text = text[:runtime_start] + text[runtime_end + len("\n})();"):]

    synthetic = '''    if (!chats.some(chat => chat.session_id === zbranoChatSessionId)) {
      chats.unshift({session_id: zbranoChatSessionId, title: "New chat", updated_at: Date.now() / 1000});
    }
'''
...
'''
    ...
    if last_script < 0:
        raise RuntimeError("ZBRANO v0.11.10 patch missing: final script close")
...
def patch_main() -> None:
    ...
    if start < 0 or end < 0:
        raise RuntimeError("ZBRANO v0.11.10 patch missing: catalog parser bounds")
    ...
    if start < 0 or end < 0:
        raise RuntimeError("ZBRANO v0.11.10 patch missing: catalog fetch bounds")
...
def verify() -> None:
    ...
    if "chats.unshift({session_id: zbranoChatSessionId" in index:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.10 verification failed: " + ", ".join(missing)
        )
