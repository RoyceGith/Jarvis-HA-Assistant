...
def req(text, old, new, label):
    if old not in text:
        raise RuntimeError(f'ZBRANO v0.10.0 patch missing: {label}')
...
async def discover_plugin_tools(url,token=""):
    ...
    async with httpx.AsyncClient(timeout=PLUGIN_TIMEOUT,follow_redirects=False) as client:
        r=await client.post(url,headers=headers,json={"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-06-18","capabilities":{},"clientInfo":{"name":"ZBRANO Plugin Manager","version":"0.10.0"}}})
