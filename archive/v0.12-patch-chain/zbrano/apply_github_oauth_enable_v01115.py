...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f'ZBRANO v0.11.15 patch missing: {label}')
...
def patch_main() -> None:
    ...
    old = (
        '                "GitHub one-click OAuth requires ZBRANO to have a registered GitHub "\n'
        '                "OAuth/GitHub App client ID. This build has no centrally registered ZBRANO "\n'
...
def patch_index() -> None:
    ...
    old = '''    if (item.auth_mode === "github-oauth" && !item.oauth_available && install) {
      ...
      install.title =
        "This ZBRANO build does not yet include a registered ZBRANO GitHub OAuth application.";
    }'''
    new = '''    if (item.auth_mode === "github-oauth" && install) {
      if (item.oauth_available) {
        install.disabled = false;
        install.textContent = "Connect GitHub";
        install.title = "Sign in to GitHub, authorize ZBRANO, then install this plugin disabled by default.";
      } else {
        install.disabled = true;
        install.textContent = "GitHub OAuth not configured";
        install.title =
          "Set github_oauth_client_id in the ZBRANO add-on configuration using an app with Device Flow enabled.";
      }
    }'''
    require(text, old, 'GitHub catalog button state')
    text = text.replace(old, new, 1)

    old_status = '"GitHub OAuth is unavailable in this build because ZBRANO does not yet have a registered GitHub OAuth application."'
    new_status = '"GitHub OAuth is not configured. Set github_oauth_client_id in the ZBRANO add-on configuration, then reload the catalog."'
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError('ZBRANO v0.11.15 verification failed: ' + ', '.join(missing))
