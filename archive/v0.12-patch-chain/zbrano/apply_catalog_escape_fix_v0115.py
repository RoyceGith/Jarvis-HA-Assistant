...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.5 patch missing: {label}")
...
'''
    ...
    if card_start < 0 or card_end < 0:
        raise RuntimeError("ZBRANO v0.11.5 patch missing: catalogCard boundaries")

    card = text[card_start:card_end]
    if "esc(" not in card:
        raise RuntimeError("ZBRANO v0.11.5 patch missing: catalog esc dependency")
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.5 verification failed: " + ", ".join(missing)
        )
