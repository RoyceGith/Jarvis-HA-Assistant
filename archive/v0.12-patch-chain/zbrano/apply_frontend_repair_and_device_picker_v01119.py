...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.19 patch missing: {label}")
...
def patch_index() -> None:
    ...
    old_send = 'socket.send(JSON.stringify({session_id: zbranoChatSessionId, message}));'
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError("ZBRANO v0.11.19 verification failed: " + ", ".join(missing))
