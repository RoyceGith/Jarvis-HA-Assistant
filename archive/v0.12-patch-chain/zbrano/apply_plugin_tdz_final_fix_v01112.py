...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.12 patch missing: {label}")
...
async function loadPlugins(){
  const listNode=currentPluginList();'''
    ...
    if loader_pos < 0 or install_pos < 0:
        raise RuntimeError("ZBRANO v0.11.12 patch missing: plugin loader bounds")
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.12 verification failed: " + ", ".join(missing)
        )
