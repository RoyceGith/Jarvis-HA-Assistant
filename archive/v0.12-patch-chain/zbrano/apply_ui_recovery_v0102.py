...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.10.2 patch missing: {label}")
...
'''
    ...
    recovery_script = r'''
<script id="zbrano-v0102-tab-recovery">
(() => {
  ...
  async function recoverEntities() {
    ...
    try {
      ...
      if (rows && rows.children.length === 0 && summary && !/0 total/i.test(summary.textContent || "")) {
        summary.textContent = "No entity rows were rendered. Select Refresh or check the ZBRANO add-on logs.";
      }
    } catch (error) {
      if (summary) {
  ...
  window.zbranoActivatePanel = activatePanel;
})();
</script>
'''
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError("ZBRANO v0.10.2 verification failed: " + ", ".join(missing))
