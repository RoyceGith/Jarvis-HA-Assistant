...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.11 patch missing: {label}")
...
'''
    ...
    new_message = '''                "GitHub one-click OAuth requires ZBRANO to have a registered GitHub "
  ...
  catalogCard = function(item) {
    ...
    if (item.installable === false || !item.url) {
      if (install) {
        ...
        install.title = "Discovered in the MCP Registry, but this entry has no remote HTTPS endpoint for ZBRANO to install.";
      }
      if (item.package_ref) {
...
'''
    ...
    if last_script < 0:
        raise RuntimeError("ZBRANO v0.11.11 patch missing: final script close")
