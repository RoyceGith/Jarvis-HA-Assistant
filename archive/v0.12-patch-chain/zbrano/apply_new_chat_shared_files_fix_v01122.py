...
def must(text, marker, label):
    if marker not in text:
        raise RuntimeError(f'ZBRANO v0.11.22 patch missing: {label}')
...
def verify():
    ...
    if 'id="clear-chat"' in index:
        raise RuntimeError('ZBRANO v0.11.22 verification failed: legacy Entities New Chat remains')
    ...
    if 'if (activeRequest) return;' in index[start:end]:
        raise RuntimeError('ZBRANO v0.11.22 verification failed: New Chat still silently returns')
