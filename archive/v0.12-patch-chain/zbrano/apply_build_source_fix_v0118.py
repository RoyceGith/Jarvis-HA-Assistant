...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.8 patch missing: {label}")
...
def main() -> None:
    ...
    if last_script < 0:
        raise RuntimeError("ZBRANO v0.11.8 patch missing: final script close")
    ...
    for old, new in candidates:
        ...
        raise RuntimeError("ZBRANO v0.11.8 patch missing: backend version statement")
    ...
    for old, new in candidates:
        ...
        raise RuntimeError("ZBRANO v0.11.8 patch missing: JSON version statement")
    ...
    for old, new in candidates:
        ...
        raise RuntimeError("ZBRANO v0.11.8 patch missing: HUD version statement")
    ...
    if '''    if "0.11.6" not in main:''' in text:
        ...
        raise RuntimeError("ZBRANO v0.11.8 patch missing: version verification")
    ...
    if '''        missing.append("backend version 0.11.6")''' in text:
        ...
        raise RuntimeError("ZBRANO v0.11.8 patch missing: verification message")
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.8 verification failed: " + ", ".join(missing)
        )
