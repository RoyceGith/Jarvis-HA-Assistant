...
ROOT = Path(os.environ.get("ZBRANO_ROOT", "/opt/zbrano"))
...
'''
    ...
    backend = replace_once(
        backend,
        'async def _run_zbrano_stream_events(message: str, session_id: str = "default") -> AsyncIterator[bytes]:\n    yield stream_event("status", message="Thinking…")',
    ...
    stream_start = backend.find('async def _run_zbrano_stream_events(')
    stream_end = backend.find('\n\nasync def run_zbrano_stream(', stream_start)
    ...
    backend = replace_once(
        backend,
        '''async def run_zbrano_stream(message: str, session_id: str = "default") -> AsyncIterator[bytes]:
    """Persist a completed streamed exchange while forwarding events unchanged."""
    reply_parts: list[str] = []
    completed = False
    async for event_bytes in _run_zbrano_stream_events(message, session_id):''',
    ...
    backend = replace_once(
        backend,
        'async for event_bytes in run_zbrano_stream(effective_message, request.session_id):',
        'async for event_bytes in run_zbrano_stream(effective_message, request.session_id, request.search_mode):',
        "WebSocket search mode",
    )
    backend = replace_once(
        backend,
        'async for event in run_zbrano_stream(effective_message, request.session_id):',
    ...
    "web_search": {
        ...
        "files": (
            "zbrano/apply_native_web_search_v01226.py",
            "zbrano/app/main.py",
...
const webSearchContextSize = document.getElementById("web-search-context-size");''',
  ...
  return escapeHtml(text)
    ...
    frontend = replace_once(
        ...
        '''  .diagnostic-return-banner button { flex: 0 0 auto; }
  .message.zbrano a { color: var(--cyan); text-decoration: underline; text-underline-offset: .15em; overflow-wrap: anywhere; }
