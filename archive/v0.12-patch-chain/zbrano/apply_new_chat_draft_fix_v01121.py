...
def must(text, marker, label):
    if marker not in text:
        raise RuntimeError(f'ZBRANO v0.11.21 patch missing: {label}')
...
def verify():
    ...
    must(index, 'showPanel("chat");\n  zbranoChatSessionId = createSessionId();', 'draft-first New Chat')
    must(index, 'localStorage.setItem("zbrano_chat_session_id", zbranoChatSessionId);', 'draft session persistence')
    ...
    if 'fetch("api/chats"' in block or 'body: JSON.stringify({session_id: sessionId})' in block:
        raise RuntimeError('ZBRANO v0.11.21 verification failed: New Chat still persists before first message')
