...
def patch_frontend(frontend: str) -> str:
    ...
    body.zbrano-input-active #brain-network { opacity: calc(var(--neural-opacity) * .1) !important; }
...
'''
    ...
    load_marker = '    reducedMotionSetting.checked = Boolean(zbranoPreferences.reduced_motion);'
    frontend = replace_once(
        frontend,
        load_marker,
    ...
    theme_listener = '  window.addEventListener("zbrano-theme-change", () => {\n    if (frame) window.cancelAnimationFrame(frame);\n    draw(performance.now(), true);\n  });'
    frontend = replace_once(
        frontend,
        theme_listener,
        theme_listener + r'''
