...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.9 patch missing: {label}")
...
(() => {
  ...
  document.addEventListener("click", async event => {
    ...
    if (newChat) {
      ...
      zbranoChatSessionId = crypto.randomUUID
        ? crypto.randomUUID()
      ...
      if (messagesNode) {
        messagesNode.innerHTML =
          '<div class="message zbrano">ZBRANO intelligence core online.</div>';
      }
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.9 verification failed: " + ", ".join(missing)
        )
