...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.3 patch missing: {label}")
...
def patch_index() -> None:
    ...
    if text.count(old_catalog_parent) < 2:
        raise RuntimeError("ZBRANO v0.11.3 patch missing: catalog fallback parents")
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.3 verification failed: " + ", ".join(missing)
        )
