...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f'ZBRANO v0.11.16 patch missing: {label}')
...
def patch_index() -> None:
    ...
    require(text, '.zbrano::before { content: "ZBRANO";', 'chat assistant label')
    text = text.replace('.zbrano::before { content: "ZBRANO";', '.zbrano::before { content: "ZBRANO";', 1)
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError('ZBRANO v0.11.16 verification failed: ' + ', '.join(missing))
