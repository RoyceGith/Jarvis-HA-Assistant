...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.6 patch missing: {label}")
...
def patch_main() -> None:
    ...
    if function_start < 0 or function_end < 0:
        raise RuntimeError("ZBRANO v0.11.6 patch missing: catalog function bounds")
    ...
    if invalid_returns:
        raise RuntimeError(
            "ZBRANO v0.11.6 found inconsistent catalog returns: "
...
def patch_index() -> None:
    ...
    body.zbrano-input-active #brain-network {
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.6 verification failed: " + ", ".join(missing)
        )
