...
'''
    ...
    run_start = '''async def run_zbrano(message: str, session_id: str = "default") -> dict[str, Any]:
    local_result = None if developer_mode_enabled() else await try_local_ha_route(message, session_id)'''
    run_replacement = '''async def run_zbrano(message: str, session_id: str = "default") -> dict[str, Any]:
    ...
    stream_start = '''async def _run_zbrano_stream_events(message: str, session_id: str = "default") -> AsyncIterator[bytes]:
    yield stream_event("status", message="Thinking…")

    pending_approval = PENDING_MCP_APPROVALS.get(session_id)'''
    stream_replacement = '''async def _run_zbrano_stream_events(message: str, session_id: str = "default") -> AsyncIterator[bytes]:
    yield stream_event("status", message="Thinking…")
