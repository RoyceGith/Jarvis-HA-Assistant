...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.2 patch missing: {label}")
...
def patch_index() -> None:
    ...
    if panel_start < 0:
        raise RuntimeError("ZBRANO v0.11.2 patch missing: plugins panel")
    panel_end = text.find("</section>", panel_start)
    if panel_end < 0:
        raise RuntimeError("ZBRANO v0.11.2 patch missing: plugins panel close")
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.2 verification failed: " + ", ".join(missing)
        )
