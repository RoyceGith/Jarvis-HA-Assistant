...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.14 source repair missing: {label}")
...
def main() -> None:
    ...
    for old_return in old_returns:
        ...
        raise RuntimeError("ZBRANO v0.11.13 patch missing: catalog response")'''
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.14 source repair verification failed: "
            + ", ".join(missing)
        )
