...
def require(text: str, marker: str, label: str) -> None:
    if marker not in text:
        raise RuntimeError(f"ZBRANO v0.11.13 patch missing: {label}")
...
def patch_index() -> None:
    ...
    if (button.dataset.oauthAvailable !== "1") {
      ...
      if (status) {
        status.textContent =
          "GitHub OAuth is unavailable in this build because ZBRANO does not yet have a registered GitHub OAuth application.";
      }
      return;
    }
...
def verify() -> None:
    ...
    if missing:
        raise RuntimeError(
            "ZBRANO v0.11.13 verification failed: " + ", ".join(missing)
        )
