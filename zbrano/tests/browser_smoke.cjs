...
const onboardingFixture = {
  ...
  installation_report: {
    generated_at: 1788300000, version: "0.13.226", ready: true, attention_count: 0, ready_count: 5,
    checks: [
...
function apiFixture(url, method = "GET") {
  ...
  if (pathname === "/api/release-memory-sync") {
    return {enabled: false, state: "disabled", version: "0.13.226", task_active: false};
  }
...
async function main() {
  ...
  try {
    ...
    await page.locator('[data-composer-plugin="plugin-1"] img.loaded').waitFor();
    assert.ok(await page.locator('[data-composer-plugin="plugin-1"] img').evaluate(image => image.naturalWidth > 0));
