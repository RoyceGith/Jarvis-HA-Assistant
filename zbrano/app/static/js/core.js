let zbranoChatSessionId = localStorage.getItem("zbrano_chat_session_id") ||
  (crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random()}`);
localStorage.setItem("zbrano_chat_session_id", zbranoChatSessionId);
...
const VOICE_SETTINGS_KEY = "zbrano_voice_settings_v1";
...
async function applyChatAutoSpeakPreference(sessionId, serverPreference) {
  if (typeof serverPreference === "boolean") {
    ...
    chatAutoSpeakPreferences[sessionId] = zbranoPreferences.auto_speak !== false;
  }
...
async function loadAgentControls(preferences = null) {
  try {
    ...
    const composerModelLabel = agentModel.parentElement.querySelector("span");
    if (composerModelLabel) composerModelLabel.textContent = `Chat model · ${provider}`;
    ...
    window.dispatchEvent(new CustomEvent("zbrano-agent-controls-loaded"));
  } catch {
...
async function saveAgentControls() {
  ...
  zbranoPreferences = {...zbranoPreferences, ...(data.preferences || {})};
...
autoSpeak.addEventListener("change", () => {
  setChatAutoSpeakPreference(zbranoChatSessionId, autoSpeak.checked);
});
...
function isQuietHours() {
  if (!zbranoPreferences.quiet_hours_enabled) return false;
  const now = new Date();
  const current = now.getHours() * 60 + now.getMinutes();
  const [startHour, startMinute] = String(zbranoPreferences.quiet_hours_start || "22:00").split(":").map(Number);
  const [endHour, endMinute] = String(zbranoPreferences.quiet_hours_end || "07:00").split(":").map(Number);
...
function applySpeechPlaybackSettings(audio){
  const rate=speechPlaybackRate;audio.volume=Number(zbranoPreferences.voice_volume??.9);audio.defaultPlaybackRate=rate;audio.playbackRate=rate;audio.preservesPitch=true;
...
function interruptActiveResponseForVoice(){
  ...
  if(request){
    request.stopped=true;const {socket,zbranoMessage}=request;
    if(socket.readyState===WebSocket.OPEN)socket.send(JSON.stringify({type:"stop"}));else socket.close();
    finishInterruptedMessage(zbranoMessage,"[Interrupted for voice]","Interrupted for voice.");
...
const PROMPT_HISTORY_KEY = "zbrano_prompt_history_v1";
...
let zbranoPreferences = {};
const THEME_KEY = "zbrano_theme_v1";
...
function applyTheme(theme, persist = true) {
  ...
  window.dispatchEvent(new CustomEvent("zbrano-theme-change", {detail: {theme: nextTheme}}));
}
...
const ENTITY_ALIAS_BACKUP_KEY = "zbrano_entity_aliases_v1";
...
function showChatWelcome() {
  ...
  addMessage("ZBRANO intelligence core online.", "zbrano");
}
...
async function refreshChatList() {
  try {
    ...
    for (const chat of chats) {
      ...
      row.className = `chat-list-item${chat.session_id === zbranoChatSessionId ? " active" : ""}`;
...
async function openChat(sessionId) {
  ...
  try {
    ...
    for (const message of restoredMessages) {
      addMessage(message.content, message.role === "user" ? "user" : "zbrano", message.attachments || []);
    }
    ...
    showChatWelcome();
    addMessage(`Could not restore chat: ${error.message || error}`, "zbrano");
  }
}
...
function renderDraftChatRow() {
  ...
  row.dataset.sessionId = zbranoChatSessionId;
...
async function createNewChat() {
  ...
  showPanel("chat");
  zbranoChatSessionId = createSessionId();
  localStorage.setItem("zbrano_chat_session_id", zbranoChatSessionId);
  updateSessionDisplay();
  renderDraftChatRow();
  showChatWelcome();
...
async function deleteChat(sessionId) {
  if (activeRequest && sessionId === zbranoChatSessionId) return;
  ...
  if (sessionId !== zbranoChatSessionId) {
    await refreshChatList();
    return;
  }
...
async function loadSettings() {
  ...
  try {
    ...
    zbranoPreferences = data.preferences || {};
    await loadAgentControls(zbranoPreferences);
    elevenLabsModel.value = zbranoPreferences.elevenlabs_model || "eleven_flash_v2_5";
    elevenLabsSpeakerBoost.checked = zbranoPreferences.elevenlabs_speaker_boost === true;
    settingsAutoSpeak.checked = zbranoPreferences.auto_speak !== false;
    document.getElementById("proactive-voice-enabled").checked = zbranoPreferences.proactive_voice_enabled !== false;
    document.getElementById("voice-approval-enabled").checked = zbranoPreferences.voice_approval_enabled !== false;
    document.getElementById("wake-word-enabled").checked = zbranoPreferences.wake_word_enabled === true;
    document.getElementById("wake-phrase").value = zbranoPreferences.wake_phrase || "hey zbrano";
    window.dispatchEvent(new CustomEvent("zbrano-voice-preferences-loaded"));
    responseLength.value = zbranoPreferences.response_length || "balanced";
    confirmationStrictness.value = zbranoPreferences.confirmation_strictness || "standard";
    webSearchEnabled.checked = zbranoPreferences.web_search_enabled !== false;
    webSearchContextSize.value = zbranoPreferences.web_search_context_size || "medium";
    contextMessages.value = String(zbranoPreferences.context_messages ?? 20);
    retentionDays.value = String(zbranoPreferences.retention_days ?? 90);
    fastMemoryEnabled.checked = zbranoPreferences.fast_memory_enabled !== false;
    fastMemoryAutoCapture.checked = zbranoPreferences.fast_memory_auto_capture !== false;
    fastMemoryContextItems.value = String(zbranoPreferences.fast_memory_context_items ?? 10);
    preferredLanguage.value = ["auto", "English", "Greek", "Italian", "French"].includes(zbranoPreferences.preferred_language) ? zbranoPreferences.preferred_language : "auto";
    window.ZbranoI18n?.setPreference(preferredLanguage.value);
    renderInterfaceLanguageFlag();
    pronunciationDictionary.value = zbranoPreferences.pronunciation_dictionary || "";
    neuralStyle.value = zbranoPreferences.neural_style || "constellation";
    neuralScale.value = String(zbranoPreferences.neural_scale ?? 1);
    neuralNodeSize.value = String(zbranoPreferences.neural_node_size ?? 1);
    neuralOpacity.value = String(zbranoPreferences.neural_opacity ?? .38);
    releaseMemoryAutoSync.checked = zbranoPreferences.auto_sync_releases_to_workshop_memory !== false;
    reducedMotionSetting.checked = Boolean(zbranoPreferences.reduced_motion);
    textSize.value = zbranoPreferences.text_size || "medium";
    interfaceDensity.value = zbranoPreferences.interface_density || "comfortable";
    quietHoursEnabled.checked = Boolean(zbranoPreferences.quiet_hours_enabled);
    quietHoursStart.value = zbranoPreferences.quiet_hours_start || "22:00";
    quietHoursEnd.value = zbranoPreferences.quiet_hours_end || "07:00";
    voiceVolume.value = String(zbranoPreferences.voice_volume ?? 0.9);
    voiceVolumeValue.textContent = `${Math.round(Number(voiceVolume.value) * 100)}%`;
    applyInterfacePreferences(zbranoPreferences);
    saveVoiceSettings();
...
saveSettings.addEventListener("click", async () => {
  ...
  try {
    ...
    zbranoPreferences = data.preferences || zbranoPreferences;
    settingsAutoSpeak.checked = zbranoPreferences.auto_speak !== false;
    window.ZbranoI18n?.setPreference(zbranoPreferences.preferred_language || preferredLanguage.value || "auto");
    applyInterfacePreferences(zbranoPreferences);
    saveVoiceSettings();
...
preferredLanguage.addEventListener("change", async () => {
  ...
  zbranoPreferences = {...zbranoPreferences, preferred_language: interfaceLanguage};
...
stopButton.addEventListener("click", () => {
  ...
  activeRequest.stopped = true;
  const {socket, zbranoMessage} = activeRequest;
  if (socket.readyState === WebSocket.OPEN) {
    socket.send(JSON.stringify({type: "stop"}));
  } else {
    socket.close();
  }
  finishInterruptedMessage(zbranoMessage, "[Response stopped]", "Response stopped.");
...
form.addEventListener("submit", async (event) => {
  ...
  input.value = "";
  resizeComposer();
  document.body.classList.remove("zbrano-input-active");
  resetToolTimeline();
  ...
  let opened = false;
  const requestState = {socket, zbranoMessage, stopped: false};
  activeRequest = requestState;

  const connectionTimer = window.setTimeout(() => {
    if (!opened && socket.readyState !== WebSocket.OPEN) {
      renderMessageContent(zbranoMessage, "Request failed: WebSocket did not connect. Check the ZBRANO app log.");
      socket.close();
    }
  }, 8000);
  ...
  socket.addEventListener("open", () => {
    ...
    socket.send(JSON.stringify({
      session_id: zbranoChatSessionId,
      message,
  ...
  socket.addEventListener("message", (messageEvent) => {
    ...
    try {
      ...
      renderMessageContent(zbranoMessage, `Invalid stream event: ${messageEvent.data}`);
      return;
    }
    ...
    if (eventData.type === "activity") {
      ...
      if (!answer) renderMessageContent(zbranoMessage, statusText);
    } else if (eventData.type === "delta") {
      markFirstResponse();
      setResponseActivity("Responding…");
      if (!answer) renderMessageContent(zbranoMessage, "");
      const followLatest = isNearMessagesBottom();
      const delta = eventData.text || "";
      answer += delta;
      speechBuffer += delta;
      renderMessageContent(zbranoMessage, answer);
      ...
      renderMessageContent(zbranoMessage, `Request failed: ${eventData.message || "Unknown error"}`);
    } else if (eventData.type === "stopped") {
      if (!String(zbranoMessage.dataset.rawText || "").includes("[Response stopped]")) {
        finishInterruptedMessage(zbranoMessage, "[Response stopped]", "Response stopped.");
      }
    }
  });
  ...
  socket.addEventListener("error", () => {
    ...
    if (!answer) {
      renderMessageContent(zbranoMessage, "Request failed: WebSocket connection error");
    }
  });

  socket.addEventListener("close", (closeEvent) => {
    window.clearTimeout(connectionTimer);
    if (!requestState.stopped && !answer && !zbranoMessage.textContent.startsWith("Request failed:")) {
...
function startBrainNetwork() {
  ...
  window.addEventListener("zbrano-theme-change", () => {
...
checkHealth();
loadSettings().finally(() => openChat(zbranoChatSessionId));
