...
class ReleaseMemoryWriteVerificationTests(unittest.TestCase):
    def test_release_markers_are_aligned(self):
        self.assertIn('version: "0.13.36"', CONFIG)
        self.assertIn('version="0.13.36"', MAIN)
        self.assertIn("HUD 0.13.36", INDEX)
        self.assertEqual(MANIFEST["version"], "0.13.36")
    ...
    def test_prior_canonical_release_descriptions_are_backfilled_in_order(self):
        ...
        self.assertEqual([item["version"] for item in records], [f"0.13.{index}" for index in range(36)])
        ...
        self.assertIn("zbrano-release:0.13.35", entries[-1])
