...
class ReleaseMemoryWriteVerificationTests(unittest.TestCase):
    def test_release_markers_are_aligned(self):
        self.assertIn('version: "0.13.26"', CONFIG)
        self.assertIn('version="0.13.26"', MAIN)
        self.assertIn("HUD 0.13.26", INDEX)
        self.assertEqual(MANIFEST["version"], "0.13.26")
    ...
    def test_prior_canonical_release_descriptions_are_backfilled_in_order(self):
        ...
        self.assertEqual([item["version"] for item in records], [f"0.13.{index}" for index in range(26)])
        ...
        self.assertIn("zbrano-release:0.13.25", entries[-1])
