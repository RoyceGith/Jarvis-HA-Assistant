...
class ReleaseMemoryWriteVerificationTests(unittest.TestCase):
    def test_release_markers_are_aligned(self):
        self.assertIn('version: "0.13.93"', CONFIG)
        self.assertIn('version="0.13.93"', MAIN)
        self.assertIn("HUD 0.13.93", INDEX)
        self.assertEqual(MANIFEST["version"], "0.13.93")
    ...
    def test_prior_canonical_release_descriptions_are_backfilled_in_order(self):
        ...
        self.assertEqual([item["version"] for item in records], [f"0.13.{index}" for index in range(93)])
        ...
        self.assertEqual(len(entries), 93)
        self.assertIn("zbrano-release:0.13.0", entries[0])
        self.assertIn("zbrano-release:0.13.92", entries[-1])
