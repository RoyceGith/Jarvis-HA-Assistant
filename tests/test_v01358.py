...
class ContainerBuildFixtureReleaseTests(unittest.TestCase):
    def test_release_markers_are_aligned(self):
        self.assertIn('version: "0.13.65"', CONFIG)
        self.assertIn('version="0.13.65"', MAIN)
        self.assertIn("HUD 0.13.65", HTML)
        self.assertEqual(MANIFEST["version"], "0.13.65")

    def test_docker_only_fixtures_match_runtime(self):
        self.assertNotIn("0.13.56", ASGI_TEST)
        self.assertNotIn("0.13.56", BROWSER_TEST)
        self.assertIn('"version"], "0.13.65"', ASGI_TEST)
        self.assertIn('version: "0.13.65"', BROWSER_TEST)

    def test_release_history_includes_v01357(self):
        self.assertEqual(MANIFEST["history_backfill"][-1]["version"], "0.13.64")
